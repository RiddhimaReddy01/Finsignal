...
METRIC_SYNS: Dict[str, List[str]] = {
    ...
    "capex": ["capex", "capital expenditures", "capital expenditure", "purchases of property and equipment"],
    "eps": ["earnings per share", "eps", "diluted eps", "basic eps"],
    "cfo": ["cash provided by operating activities", "operating cash flow", "net cash from operating activities"],
...
def verify_evidence(
    ...
    *,
    ...
    if signals["n_blocks"] < req.min_total_blocks:
        return VerificationResult(
            status="abstain",
            confidence=0.20,
            mode=plan.mode,
    ...
    if n_sources_present < req.min_sources:
        return VerificationResult(
            status="abstain",
            confidence=0.20,
            mode=plan.mode,
    ...
    if signals["n_chunks"] < req.min_chunks:
        return VerificationResult(
    ...
    if signals["n_tables"] < req.min_tables:
        return VerificationResult(
            status="answer_with_warning",
            confidence=0.58,
            mode=plan.mode,
    ...
    if req.min_item1a_like and signals["n_item1a_like"] < req.min_item1a_like:
        return VerificationResult(
            status="answer_with_warning",
            confidence=0.57,
            mode=plan.mode,
